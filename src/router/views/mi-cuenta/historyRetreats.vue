...
export default {
  ...
  mounted() {
    this.fetchTransactionListbyId({
      ...
      this.retreatsDataHistory = res.transaccion.filter(item => item.type_transactions === 'retiro')
      this.dataBackup = res.transaccion.filter(item => item.type_transactions === 'retiro')
              ...
              <table class="table table-nowrap table-centered mb-0 align-middle">
                ...
                <tbody>
                  <tr v-for="item in retreatsDataHistory" :key="item._id">
                    ...
                    <td>
                      <div v-if="item.banco !== 'N/A'" class="text-muted text-capitalize">{{item.banco}}</div>
                      <div v-if="item.banco === 'N/A' && item.metodo_transactions === 'wallet'" class="text-muted text-capitalize">{{item.network}}</div>
                    </td>
                    <td>
